...
void PixTestXray::doPhRun() {
  ...
  // -  This part seems to be important for the pxar gui
  gStyle->SetPalette(1);
  PixTest::update();
  fDirectory->cd();

  // -- unmask entire chip and then mask hot pixels
  fApi->_dut->testAllPixels(false); //  Testing all pixels on all rocs,
  fApi->_dut->maskAllPixels(false); // Turn all pixels on
  for (unsigned int i = 0; i < fHotPixels.size(); ++i) { // HotPixels which fails the Alive test
    ...
    for (unsigned int ipix = 0; ipix < hot.size(); ++ipix) {
      ...
      fApi->_dut->maskPixel(hot[ipix].first, hot[ipix].second, true, getIdFromIdx(i)); // We turn off all these pixels.
    }
  }
  maskPixels(); // Look inside the getConfigParameters to find HotPixels from previous tests, and mask them as well


  vector<uint8_t> rocIds = fApi->_dut->getEnabledRocIDs(); // Function returning the Ids of all enabled ROCs in a uint8_t vector, not important for us, because we only havea single ROC

  int totalPeriod = prepareDaq(fParTriggerFrequency, 50); // fParTriggerFrequency is set by user in the gui (fParTriggerFrequency = trgfrequency), http://www.ni.com/white-paper/13655/en/, 50 is trigger delay, calulates the period of the trigger
  ...
  if (fQ.size() > 0 && (fSourceChanged || fHistList.size() == 0)) { //fQ is a 1dim Histogram in root, fSourceChanged is a boolien and checks if there is any changes inside the file (not sure which file)
    if (fSourceChanged) {
      LOG(logDEBUG) << "booking new histograms as source name has changed"; // Writting into Logfile
    } else {
      LOG(logDEBUG) << "booking new histograms";
    }
    fQ.clear(); // Reset previous data
    fQmap.clear();
    fHmap.clear();
    fPHmap.clear();
    fPH.clear();
  ...
  if (0 == fQ.size()) { // Here the histograms get configurated i.e. get labels and titles
    fSourceChanged = false;
    if (fParFillTree) bookTree();
    TH1D *h1(0);
    TH2D *h2(0);
  ...
  uint8_t perFull;
  TStopwatch t; // creating a TStopwatch opject,  TStopwatch.h This class returns the real and cpu time between the start and stop events
  fApi->daqStart(FLAG_DUMP_FLAWED_EVENTS); // FLAG_DUMP_FLAWED_EVENTS means Flag to dump all flawed events plus the surrounding ones to the logger on ERROR level. daqStart is a Function to set up and initialize a new data acquisition session
  ...
  while (fApi->daqStatus(perFull) && fDaq_loop) { //daqStatus change value of perrFull (see api.cc)
    ...
    if (perFull > 80) { // This if testing if buffer is full, i guess, 80 could be 80 percent -> almoust full
      ...
      processData(0); // Collect hits, and fill the histograms
  ...
  finalCleanup();
  fQ[0]->Draw(); // Draw Histograms
  fDisplayedHist = find(fHistList.begin(), fHistList.end(), fQ[0]);  //Updating pxar software
  PixTest::update();
